...
open class PriceButton: UIButton {
    ...
    public convenience init(
        ...
        action: ((_ sender: PriceButton) -> Void)? = nil
        ) {
    ...
    @objc open var action: ((_ sender: PriceButton) -> Void)? {
        didSet {
            addTarget(self, action: #selector(toggle), for: .touchUpInside)
        }
    }
    ...
    private func setupCurrentState() {
        ...
        UIView.transition(with: self, duration: 0.5, options: .transitionCrossDissolve, animations: { [weak self] in
            self?.setTitle(currentTitle.isEmpty ? nil : " " + currentTitle, for: UIControlState())
            self?.setTitleColor(self?.currentColor ?? self?.tintColor, for: UIControlState())
            self?.backgroundColor = self?.currentBackgroundColor ?? .clear
            self?.setImage(self?.currentToggleImage ?? self?.currentImage, for: UIControlState())
            }, completion: nil)
    }
