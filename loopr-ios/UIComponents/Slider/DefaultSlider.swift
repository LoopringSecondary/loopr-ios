...
@IBDesignable open class DefaultSlider: UIControl {
    ...
    public convenience init(frame: CGRect = .zero, completion: ((DefaultSlider) -> Void)? = nil) {
    ...
    open weak var delegate: DefaultSliderDelegate?
    ...
    private var leftHandleAccessibilityElement: UIAccessibilityElement {
        let element: DefaultSliderLeftElement = DefaultSliderLeftElement(accessibilityContainer: self)
    ...
    private var rightHandleAccessibilityElement: UIAccessibilityElement {
        let element: DefaultSliderRightElement = DefaultSliderRightElement(accessibilityContainer: self)
        element.isAccessibilityElement = true
    ...
    /// When subclassing **DefaultSlider** and setting each item in **setupStyle()**, the design is reflected in Interface Builder as well.
    ...
    private func updateLabelValues() {
        ...
        if let replacedString = delegate?.defaultSlider(self, stringForMinValue: selectedMinValue) {
            minLabel.string = replacedString
        } else {
            minLabel.string = numberFormatter.string(from: selectedMinValue as NSNumber)
        }
        
        if let replacedString = delegate?.defaultSlider(self, stringForMaxValue: selectedMaxValue) {
    ...
    fileprivate func refresh() {
        ...
        if let delegate = delegate, handleTracking != .none {
            delegate.defaultSlider(self, didChange: selectedMinValue, maxValue: selectedMaxValue)
        }
    }
...
// MARK: - DefaultSliderLeftElement

private final class DefaultSliderLeftElement: UIAccessibilityElement {
    
    override func accessibilityIncrement() {
        guard let slider = accessibilityContainer as? DefaultSlider else { return }
...
// MARK: - DefaultSliderRightElement

private final class DefaultSliderRightElement: UIAccessibilityElement {
